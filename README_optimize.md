...
5. 本分支打开了`absorb_for_prefill`，可以考虑将其关闭：`ktransformers/optimize/optimize_rules/DeepSeek-V3-Chat.yaml`，搜索`absorb_for_prefill`改成`False`即可
6. 模型内存会使用巨页并被持久化，这样进程可以快速启动。请确保巨页文件系统已挂载到`/dev/hugepages`（ubuntu默认就会挂载到这个位置）并且确保启动ktransformers的用户有权限操作该路径
...
```shell
echo '嗯，关于用户的这个问题，我的思考应当尽量简短。如下是我的思考：' > /tmp/kt_force_think_prefix
```
