...
- **处置决策（2026-08-21，用户确认）：BCG 线搁置**——issue 草稿保留在
  `upstream-bcg-issue.md`（含复现/症状矩阵/12 项排除/诊断开关说明），
  诊断工具与两处必要修复（kt MoE 断点、KV 直写断点、NamedTuple 上游
  bug）保留在 sglang 分支（默认零开销）；日后提交上游需先推公开 fork
  （分支 `305cc43ca` 及祖先）。生产维持 BCG 禁用（506.9/47.9 现状）。
  下一个优先优化候选：按热度专家放置（见 §6.10 首轮实测 11.3%→67.0%
  路由承接、5.9× 余量，零额外显存）。
